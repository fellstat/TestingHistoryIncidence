...
bootstrap_incidence <- function(report_hiv_pos, biomarker_art, low_viral, hiv,
  ...
  if(is.null(frame) && is.null(id_formula) && is.null(strata_formula) && is.null(weights_formula)){
    ...
    bb <- srs_bootstrap(dat,
                           fun=function(dat) unlist(testing_incidence(dat$report_hiv_pos, dat$biomarker_art, dat$low_viral, dat$hiv,
                                                                    dat$last_hiv_lower, dat$last_hiv_upper, dat$ever_hiv_test, dat$weights, ...)),
    ...
    bb <- survey_bootstrap(des,
                           fun=function(dat, wts) {
                             inc <- testing_incidence(dat$report_hiv_pos, dat$biomarker_art, 
                                                      dat$low_viral, dat$hiv, dat$last_hiv_lower, 
                                                      dat$last_hiv_upper, dat$ever_hiv_test, 
                                                      wts, ...)
                             unlist(inc)
                           },
                           show_progress=show_progress,
